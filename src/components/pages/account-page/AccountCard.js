...
const AccountCardComponent = ({ quoteAccount, authorQuoteAccount,
    id, password, userTypeId, formData, changeDisabled, changeInputValue,
    ...
    const setDataUser = async (photo) => {
        ...
        for (let key in myFormData) {
            dataUser.append(key, myFormData[key])
        }

        dataUser.append('avatar', photo, photo.name)

        for (let [name, value] of dataUser) {
            console.log (`${name} = ${value}, type: ${typeof value}`)
        }
        
        await request(`http://localhost:5100/api/users/${id}`, 'PUT', dataUser,
            ...
            <div className="header-account" style={{ background: `url(${bg})` }}>
                ...
                <div className="photo">
                    ...
                    <div className="avatar">
                        ...
                        <input onChange={setPhotoHandler} type="file" name="avatar" id="photo" title="Change photo"></input>
                    </div>
